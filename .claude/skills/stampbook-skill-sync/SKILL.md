...
⚠️ **작업이 길어졌으면 푸시 직전에 main이 앞서 나갔는지 반드시 다시 확인할 것.**
사용자가 다른 세션에서 동시에 작업하는 일이 잦아서, 조사·집필에 한두 시간 쓰는 사이
main에 커밋이 여러 개 들어와 있을 수 있다. 그대로 머지하면 **그 새 작업이 통째로 되돌아간다.**

```bash
git fetch origin main
git log --oneline HEAD..origin/main     # 비어 있지 않으면 뒤처진 것
git merge origin/main                   # 뒤처졌으면 먼저 병합하고 충돌 정리
```
- 충돌이 나면 **거의 항상 main 쪽이 최신**이다. main이 지운 것을 되살리지 말고,
  main이 새로 쓴 문장·구조를 그대로 두고 **내가 더한 것만 그 위에 얹는다**
- 스킬 문서(`SKILL.md`·`template-state.md`)는 통째로 다시 쓰기 쉬운데, 그러면 남의 PR이 날아간다.
  충돌이 크면 `git checkout origin/main -- <파일>`로 **main 판을 가져온 뒤 내 수정만 다시 얹을 것**
- 병합 후 제주 쪽이면 **브라우저로 다시 한 번 확인**한다 (main이 화면 구조를 바꿔놨을 수 있다)
