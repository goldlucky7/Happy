...
| 크기 | 472501 bytes |
| 지문 (sha256 앞 16자) | `e5b9d8916a5e86f1` |
| 마지막 반영 커밋 | `b9d0bd8` 🎢 탈것·놀이기구 카테고리 23곳 (+ `ff18df3` 최신 main 병합) |
...
세부 카테고리 칩: **18개** (🎟 롯데렌터카 칩을 뺀 16개 + 🚢 배·잠수함·요트 / 🎢 탈것·놀이기구 2개 추가)

곳수 표기 네 군데(`<title>` · `.h-eyebrow` · 머리말 소개문 · 「처음이신가요?」 안내문)가 모두 318로 맞춰져 있다.

## ⚠️ main에 아직 안 올라간 상태

`b9d0bd8`(탈것 카테고리)는 **브랜치 `claude/jeju-template-category-63n8j5`에만 있고 `origin/main`에는 없다.**
즉 배포 사이트(https://goldlucky7.github.io/jeju/)에는 탈것 탭이 아직 안 보인다.

- 다음 동기화 때 `origin/main`만 보면 크기 445854 / 커밋 `ff18df3`로 나온다. 그건 새 기능이 사라진 게 아니라 **아직 머지가 안 된 것**이다
- 확인: `git -C /home/user/jeju log --oneline origin/main -1` 에 `b9d0bd8`이 보이는지
- 머지되면 위 표의 크기·지문과 main이 같아진다

## 이번에 반영한 것 — 🎢 탈것·놀이기구 카테고리 (9번째 카테고리)

제주판에 **탭 하나가 통째로 새로 생긴** 첫 사례다. 장소 추가와 달리 손댈 곳이 10군데였고,
그 절차를 `travel-stampbook/SKILL.md`의 "카테고리를 통째로 새로 만들 때" 표로 남겼다.

| 묶음 | 곳 |
|---|---|
| 🚢 배 타고 나가기 | 9 (잠수함 2 · 유람선 3 · 요트 2 · 도항선 1 · 제트보트 1) |
| 🏎 직접 몰아보는 것 | 7 (레일바이크 · 카트 · ATV 3 · 버기 · 우도 전기차) |
| 🪂 하늘로 뜨는 것 | 2 (짚라인 · 패러글라이딩) |
| 🎢 놀이기구·테마파크 | 3 |
| 🐴 말 타기 | 2 |

- 새로 생긴 이름: `RIDEG` `RIDE_GROUPS` `addRideGroups` (main 263개 → 266개, 없어진 이름 없음)
- 좌표는 **확인된 8곳만** `LL`에 넣었다. 나머지 15곳은 [내 주변]·동선 순번에서 빠진다 (의도된 상태 — 지도 사이트가 막힌 환경이라 못 구했고, 지어내지 않았다)
- 전화번호는 확인된 21곳만 `TEL`에 있다
- 용어 사전에 탈것 용어 11개 추가 (짚라인·패러글라이딩·활공장·ATV·버기카·레일바이크·도항선·외승·갑마장·세일링)
- 병합 시 주의했던 것: main이 걷어낸 `.lotte-badge`/`.lotte-line`을 되살리지 않았고, 탭 줄은 main이 정리한 한 줄 가로 스크롤 순서를 따르며 탈것만 끝에 붙였다 (내 일정·갔다온 곳은 `#myRow` 소속)
...
그룹 정의·출력 함수 + `render()` 분기 + 칩 + 곳수 표기 4곳. 자세한 표는
`travel-stampbook/SKILL.md`의 "카테고리를 통째로 새로 만들 때" 절 참고. `ride` 추가가 그 절차 그대로였다.

## 코드 구조 목록 (이 시점 기준)

여기 **없는 이름이 새로 나타나면 그게 새 기능**이다. 총 266개.
(`b9d0bd8`에서 `RIDEG` `RIDE_GROUPS` `addRideGroups` 3개가 늘었고 없어진 이름은 없다)
...
NEAR_LIMIT_KM NM PHOTO PK PKMAP PKT PK_OF PK_ORDER PLACES QR_LIBS READ_URL
RIDEG RIDE_GROUPS SCAN_CAP_LIVE SCAN_LIBS SCAN_MAX SCAN_MAX_FULL
SCAN_PASSES SHOPTIER SHOP_T SPATIER SPA_T STARS TEL TIERMAP TIMED_OUT
UPD_SEEN_KEY UPD_URL VJ WRITE_URL ZONES ZONE_KW ZONE_OF activeEff
activeZone adaptiveBin addCheapGroups addCustomPlace addEffGroups
addFoodGroups addGrid addRideGroups addRouteGrid addSubHead addTierGroups
allPlaces apIsKR apLive apName apOptions autoSync barNear bcbpShift
boostContrast byEff byPk camError carrierName cdText chipMatch chipsEl
clearNearby closeFlight copyAddr couponModal currentFilter customPlaces
customToCard daysAway decodeImageFile delFlight deleteCustomPlace distKm
drawCoupons drawUpd drawUpdFail effGrid effOf ensureJsQR ensureZXing
escHtml fetchRemote flCanvas flDraft flMsg flPass flStream flTorchOn
flightIntl flightModal flightSearch flightTimes fmtDateF fmtDay fmtDist
fmtHM foldNow frameBrightness ftOf getNativeDetector gloDef gloInput
gloModal gloSug gloWord glossify grabCanvas grabPixels handlePhoto
hasPlace hopKm hoursModal isWished isoDate jsqrCanvas jsqrRead julianToISO
list llOf loadFlights loadScript lsGet lsSet makeCard markUpdBtn
matchesEff matchesSearch mergeCustom modal myFlights nearbyBtn nearbyOn
nextPass normalize onDecoded onSelEnd online openFlight openGlo openShot
openUpd parseBCBP parseLoose parsePass passes pathLen persistFlights
pickEngine pkOf placeText planGroup pushRemote rateOf rateSrc rates
refocus render renderAirTable renderApSteps renderEffMenu renderFilterBar
...
renderZoneMenu routeNote routeOrder saveDraft saveLocal saveRate saveWish
scanCap scrollToList searchClear searchInput searchQ selBtn setAiLinks
setEngine setFlPane setTab setWishDate setupFold setupTorch shBuf shCache
sharpness showGlo sortFlights startScan stopScan syncChips syncDraft
ticketHtml toCho toggleBarNear toggleTorch toggleVisit toggleWish
torchTrack unsharp updBtn updFeed updModal updSeen updateProgress
updateSyncLabel updateVisitedTab visibleCount visited wish withTimeout
zoneGrid zoneOf zxDecodeCanvas zxHints
```
